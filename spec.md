...
- API shape (draft):
    ...
    - POST /receipts/:id/image (attach image to an imageless receipt; triggers verify-scan)
    ...
    - GET /receipts/:id/matches (candidates for the review-screen match panel; computed at read time, no stored match state)
    - POST /receipts/:id/merge-from (apply merge mechanics from a source receipt), DELETE /receipts/:id (discard; deletes image object if sole owner)
- Schema (draft):
