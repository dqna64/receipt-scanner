...
## Browse
- "Upload history": all receipts, chronological, filterable by state (scan_state and/or the derived needs_review/reviewed), date range, merchant, receipt category, item tag, payment method, tax-deductible, source (scanned/manual/recurring).
- Receipt detail view with zoomable image.

## Insights (all decided; counts_in_stats rows only — see Invariants; never summed across currencies)
...
- Schema (draft):
    ...
    - categories(id, user_id, name) — one shared user-editable taxonomy for both receipt- and item-level; ≤1 per receipt and ≤1 per item; embedded as enum in scanner output schema; referencing FKs ON DELETE SET NULL (delete → referencing rows become uncategorized, live; rename keeps the stable id)
    ...
    - receipts(id, user_id, image_key, image_sha256 nullable (normalized image; indexed, non-unique — force=true permits genuine re-uploads), store_name_raw, merchant, purchase_date, total_cents BIGINT (negative for refunds/reimbursements), currency CHAR(3) DEFAULT 'AUD', kind[purchase|refund|reimbursement|transfer] (extensible — 'income' reserved post-M1), scan_state[idle|uploaded|scanning|scan_failed], scan_attempts, reclaim_count, scan_started_at, proposed_at, reviewed_at, payment_method_id FK nullable (ON DELETE SET NULL), tax_deductible, category_id FK nullable (ON DELETE SET NULL), note, source[scanned|manual|recurring], recurring_id FK nullable (ON DELETE SET NULL), occurrence_ordinal nullable (set only on recurring-generated rows; unique (recurring_id, occurrence_ordinal) → idempotent generation), offsets_receipt_id FK nullable (refund/reimbursement → original expense; ON DELETE SET NULL), raw_ocr_json, created_at, updated_at)
      - stats/charts group by (or filter to) currency — never sum across currencies
      - index (user_id, total_cents, purchase_date) — serves the match-panel candidate query across full history; candidates are matched WITHIN the same currency (total_cents is currency-relative — 50 USD and 50 AUD both store 5000 and must not false-match)
    ...
    - recurring_transactions(id, user_id, name, merchant, kind[purchase|transfer], expected_amount_cents, currency, interval_n, interval_unit[day|week|month|year], anchor_date, occurrences_generated (occurrence ordinal k; next date = anchor + (occurrences_generated+1)×interval, clamped — see interval semantics), next_expected_date (materialized from the ordinal for the worker's due-date query), end_date nullable, remaining_count nullable, purchase_receipt_id FK nullable (installment plans → what was bought; ON DELETE SET NULL), default_payment_method_id, default_category_id, default_tax_deductible, active)
      - auto-deactivates when end_date passes or remaining_count hits 0
      - generated pending receipts carry the occurrence ordinal; a unique (recurring_id, occurrence_ordinal) makes generation idempotent (crash/double-tick safe); catch-up generates only the latest missed occurrence (see interval semantics)
...
- **Recurring generation is idempotent:** unique (recurring_id, occurrence_ordinal); ordinal tracked via occurrences_generated; catch-up collapses multiple missed occurrences to the latest.
...
- **Ownership:** every resource query carries the user_id predicate; non-owned/missing id → 404 (no existence leak). The image_sha256 dup check is scoped (user_id, image_sha256).
- **Money:** integer cents end-to-end; dedupe/match compares within a single currency; stats group by currency, never sum across.
...
- Prompt/schema design for structured extraction (provider + model decided: Claude Haiku 4.5, structured outputs; schema embeds the user's category taxonomy (receipt + item level) AND the item-level tag taxonomy as enums; must be kind-aware — return dockets scan as refunds)
